...
DESTINATION_ISSUE_QTY = Decimal("20.0000")
EXPECTED_DESTINATION_ON_HAND_QTY = Decimal("30.0000")
EXPECTED_DESTINATION_ISSUED_ORIGINAL_VALUE = Decimal("20.00000000")
EXPECTED_DESTINATION_ISSUED_CORRECTED_VALUE = Decimal("18.00000000")
EXPECTED_DESTINATION_ON_HAND_ORIGINAL_VALUE = Decimal("30.00000000")
EXPECTED_DESTINATION_ON_HAND_CORRECTED_VALUE = Decimal("27.00000000")
...
fifo_mixed = _load_module(
    filename=(
        "test_purchase_value_correction_"
        "fifo_transfer_mixed_postgresql_chronology.py"
    ),
    module_name=(
        "_ma_transfer_pvc_mixed_issue_harness"
    ),
)
...
@pytest.mark.asyncio
async def test_purchase_value_correction_moving_average_transfer_postgresql_mixed():
    """
        ...
        try:
            ...
            if d4 > period_end:
                pytest.skip(
                    ...
                    "four usable business dates inside "
            ...
            (
                destination_issue_document,
                destination_issue_line,
            ) = (
                await fifo_mixed.create_destination_fifo_issue(
                    db,
                    fixture=fixture,
                    product_id=product_id,
                    warehouse_id=destination_warehouse_id,
                    quantity=DESTINATION_ISSUE_QTY,
                    document_date=d3,
                )
            )
            ...
            destination_issue_ice = await mapping_one(
                db,
                """
                SELECT
                    id,
                ...
                """,
                {
                    ...
                    "document_id":
                        int(
                            destination_issue_document.id
                        ),
                    "document_line_id":
                        int(
                            destination_issue_line.id
                        ),
                },
            )

            destination_issue_ice_id = int(
                destination_issue_ice["id"]
            )

            assert (
                destination_issue_ice[
                    "valuation_method"
                ]
                == "weighted_average_moving"
            )
            ...
            destination_issue_movement_id = int(
                destination_issue_movement[
                    "id"
                ]
            )
            ...
            ) == EXPECTED_DESTINATION_ON_HAND_QTY
            ...
            assert q8(
                ...
                EXPECTED_DESTINATION_ON_HAND_ORIGINAL_VALUE
            )
            ...
            destination_issue_ice_snapshot = dict(
                destination_issue_ice
            )

            destination_issue_movement_snapshot = dict(
                destination_issue_movement
            )

            print(
                "D3 REAL DESTINATION MA ISSUE "
                "20 VALUE 20 = PASS"
            )

            print(
                "DESTINATION ISSUE MOVEMENT -> ICE "
                "= PASS"
            )

            # ==================================================
            # F. D4 REAL PVC 120 -> 108
            # ==================================================
            # ==================================================
            ...
            print(
                "D4 REAL PVCA 120 -> 108 = PASS"
            )

            # ==================================================
            # G. PRODUCTION MA PEER RECONCILIATION
            # ==================================================
            ...
            assert len(destination_issued_rows) == 1
            assert len(destination_on_hand_rows) == 1

            source_effect = source_rows[0]

            destination_issued_effect = (
                destination_issued_rows[0]
            )

            destination_on_hand_effect = (
                destination_on_hand_rows[0]
            )

            # SOURCE ON_HAND 70 -> 63
            assert Decimal(
                source_effect[
                    "quantity"
                ]
            ...
            assert q8(
                ...
                EXPECTED_DESTINATION_ISSUED_ORIGINAL_VALUE
            )
            ...
            assert q8(
                ...
                EXPECTED_DESTINATION_ISSUED_CORRECTED_VALUE
            )
            ...
            ) == destination_issue_movement_id

            assert int(
                destination_issued_effect[
                    "source_inventory_cost_entry_id"
                ]
            ) == destination_issue_ice_id

            # recognition_date = max(PVC D4, issue D3) = D4
            assert (
                destination_issued_effect[
                    "recognition_date"
                ]
            ...
            assert q8(
                ...
                EXPECTED_DESTINATION_ON_HAND_CORRECTED_VALUE
            )
            ...
            print(
                "PVC SOURCE ON_HAND 70 -> 63 = PASS"
            )

            print(
                "PVC DESTINATION ISSUED 20 -> 18 = PASS"
            )

            print(
                "PVC DESTINATION ON_HAND 30 -> 27 = PASS"
            )

            print(
                "PVC TOTAL VALUE 120 -> 108 = PASS"
            )

            # H. SOURCE TRANSFER ISSUE MUST NOT BE FINAL TARGET
            # ==================================================
            ...
            # ==================================================
            # I. IMMUTABLE TRANSFER / DOWNSTREAM ISSUE PROVENANCE
            # ==================================================
            ...
            # ==================================================
            destination_issue_ice_after = dict(
                await mapping_one(
                    db,
                    """
                    SELECT
            ...
            destination_issue_movement_after = dict(
                await mapping_one(
                    db,
                    """
                    SELECT
            ...
            assert (
                destination_issue_ice_after
                == destination_issue_ice_snapshot
            )

            assert (
                destination_issue_movement_after
                == destination_issue_movement_snapshot
            )

            print(
                "DOWNSTREAM ISSUE ICE / MA MOVEMENT "
                "IMMUTABILITY = PASS"
            )
    ...
    # ======================================================
    # K. OUTER TRANSACTION ROLLBACK
    # ======================================================

    after = await complete_baseline()

    assert after == baseline, (
        "\nMA Transfer PVC PostgreSQL MIXED rollback "

...
import app.services.customer_advance_clearing_journal_service as service
...
from app.services.customer_advance_clearing_journal_service import (
    ...
    get_original_customer_advance_clearing_journal_entry,
...
async def test_reversal_event_cannot_generate_original():
    ...
    with pytest.raises(
        ...
        await generate_and_post_customer_advance_clearing_journal_entry(
            db,
            event=event(
                reversal_of_id=9
            ),
...
async def test_role_resolution_error_is_wrapped(
    ...
    with pytest.raises(
        CustomerAdvanceClearingJournalError,
        match="customer_advances missing",
    ):
        await generate_and_post_customer_advance_clearing_journal_entry(
            db,
            event=event(),
...
async def test_get_original_returns_matching_entry():
    ...
    result = (
        await get_original_customer_advance_clearing_journal_entry(
            db,
            company_id=1,
            customer_advance_clearing_event_id=10,
        )
    )
...
async def test_reversal_uses_supplier_typed_source_override(
    ...
    result = (
        await reverse_customer_advance_clearing_journal_entry(
            db,
...
async def test_reversal_rejects_non_reversal_event():
    ...
    with pytest.raises(
        CustomerAdvanceClearingJournalSourceStateError,
        match="Only a Customer Advance Clearing reversal",
    ):
        await reverse_customer_advance_clearing_journal_entry(
            db,

...
def test_preserves_supplier_source_by_default():
    assert (
        _resolve_reversal_supplier_advance_clearing_event_id(
            original_supplier_advance_clearing_event_id=10,
            override=None,
        )
        == 10
    )
...
def test_none_remains_none_without_override():
    assert (
        _resolve_reversal_supplier_advance_clearing_event_id(
            original_supplier_advance_clearing_event_id=None,
            override=None,
        )
        is None
    )
...
def test_override_requires_supplier_original():
    with pytest.raises(
        AccountingReversalError,

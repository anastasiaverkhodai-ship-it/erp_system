...
async def test_real_gl_anomalies_period_boundary_company_scope_and_read_only():
    ...
    schema='test_output_control_'+uuid4().hex
    try:
        async with engine.connect() as conn:
            ...
            try:
                ...
                async with AsyncSession(conn,expire_on_commit=False) as db:
                    ...
                    result=await reconcile_output_tax_calculation_from_active_sources(db,company_id=1,
                    ...
                    async def control(start=D1,end=D4,company=1):
                        return await reconcile_output_vat_gl(db,company_id=company,date_from=start,date_to=end)
                    initial=await control()
                    assert initial.matched and initial.expected_output_vat==Decimal(12)
                        ...
                        with pytest.raises(RollbackFixture):
                            async with db.begin_nested():
                                ...
                                if code=='journal_date_mismatch':
                                    ...
                                    assert later.expected_output_vat==0 and later.posted_output_vat==12
